...
export function AdminVouchersRegister() {
  /**
   * The voucher ledger, from `/vouchers`.
   *
  ...
  const FIELDS: FormField[] = [
    {
      ...
      /* The live customer register, not a list typed out here — a name that
         ...
         The value is the customer's PUBLIC ID, never their name, and both
         halves of that matter:

           · the API resolves this field with
             `ConsoleCustomerRepository::find()`, which looks up `public_id`.
             Sending a name meant the lookup found nothing, so every voucher
             issued by hand was written with `customer_user_id = NULL` —
             attached to no account at all. The name went into the display
             column and the link to the person was silently dropped. Under the
             wallet that is worse than untidy: an owner-less voucher is treated
             as a bearer code, so a voucher addressed to one shopper could be
             added to anybody's balance.

           · names are not unique. `Select` keys its items by value, so two
             customers sharing a name produced duplicate React keys AND a
             duplicate Radix value — which makes the trigger render EVERY
             matching item's label at once, as one run-on paragraph. Nine test
             accounts all called "Codex Scroll Test" turned the field into a
             wall of text. Public ids cannot collide, so both go away.

         The label still leads with the name, because that is what an operator
         is looking for; the email disambiguates two people who share one. */
      options: customers.map((customer) => ({
        value: customer.id,
        label: `${customer.name} — ${customer.email}`,
      })),
      required: true,
      /* Asked once, when the voucher is issued, and never again. `toUpdate`
         does not send it, so the server would ignore an edit anyway — and a
         required select whose stored value is a NAME while its options are now
         ids would sit empty on the edit form and refuse to save the amount. */
      createOnly: true,
      full: true,
    },
    {
