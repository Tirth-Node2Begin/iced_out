...
import { AdminPage, Note, Section } from "@/components/shell/admin-ui";
import {
  RecordManager,
  type Column,
  type FormField,
...
export function AdminReviewModeration() {
   ...
   */
  /**
   * Moderating a review changes the SHOP as well as this register — a published
   * one appears on the product page, a hidden one leaves it.
   *
   * In the single-app console this used to call `resetPublishedReviews()` to
   * drop the storefront's cached list. That call cannot do anything from here:
   * the shop is a different origin in a different tab with its own module
   * memory, so there is no store in this process to invalidate. The storefront
   * re-reads `/catalog/products/{slug}` on its next load and picks the decision
   * up then, which is what actually happened before too — the reset only ever
   * mattered when both halves shared one bundle.
   */
